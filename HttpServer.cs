...
namespace Sherpa
{
	public class HttpServer
	{
		...
		{
			public byte IDLength;			// 0x00		Size of the Image ID field
			public byte ColorMapType;		// 0x01		Color map type
			public byte ImageType;			// 0x02		Image type code
			public short CMapStart;			// 0x03		Color map origin
			public short CMapLength;		// 0x05		Color map length
			public byte CMapDepth;			// 0x07		Depth of color map
			public short XOffset;			// 0x08		X origin of the image
			public short YOffset;			// 0x0A		Y origin of the image
			public short Width;				// 0x0C		Width of the image
			public short Height;			// 0x0E		Height of image
			public byte  PixelDepth;		// 0x10		Bits per pixel
			public byte  ImageDescriptor;	// 0x11		Image descriptor byte
		};

/*
		public partial class TGAReadStream : FileStream
		{
			int Span;

			public TGAReadStream(string filename, FileMode filemode) : base(filename, filemode)
			{
			}

			override public int Read(byte[] buffer, int offset, int count)
			{
				int hdrsz = Marshal.SizeOf(typeof(TGAHeader));

				if ((base.Position == 0) && (offset == 0) && (count >= hdrsz))
				{
					byte lo, hi;

					base.Read(buffer, 0, hdrsz);
					lo = buffer[0xE];
					hi = buffer[0xF];
					ushort height = (ushort)((ushort)(hi << 8) | lo);
					height /= 2;
					lo = (byte)(height & 0xFF);
					hi = (byte)((height >> 8) & 0xFF);
					buffer[0xE] = lo;
					buffer[0xF] = hi;

					// get the number of bits per pixel; divide by 8 for bytes per pixel
					int bpp = (int)buffer[0x10];
					bpp /= 8;

					lo = buffer[0xC];
					hi = buffer[0xD];
					ushort width = (ushort)((ushort)(hi << 8) | lo);

					// the Span is the image width x bytes per pixel
					Span = width * bpp;
				...
				// Read a scan line
				int ret = base.Read(buffer, 0, Span);

				// Skip the next scan line
				if (ret == Span)
					base.Position += Span;
		...
		private void ListenerRequestCallback(IAsyncResult result)
		{
			...
			else
			{
				...
				/*
								...
								{
									// eventually this should be abstracted to handle any file types
									// but for the moment, this will load our TGA files so we 
									// can halve the vertical resolution and skip alternating lines
									// to improve load performance
									datastream = new TGAReadStream(filename, FileMode.Open);
								}
				*/
				datastream = File.OpenRead(filename);
